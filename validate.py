...
Q.forEach((q,i)=>{
  ...
  fields.forEach(([f,t])=>{
    ...
    const outside=o.replace(/<span class="[ms]">[\s\S]*?<\/span>/g,"");
    if(/[≥≤]|&lt;|&gt;/.test(outside))
...
const GOLD=[
 ...
 ["متباينة بمتغيّر عربي تبقى بترتيب العربية", "إذا كان 2س + 3 ≥ 11، فما أصغر قيمة؟", null],
 ["مقارنة بحروف تسمية لا تُعزل ككتلة", "إذا كان أ > ب > 0، قارن", null],
 ["نسبة بحروف عربية لا تُعزل ككتلة", "س : ص = 2 : 7 وكان", null],
 ["نسبة أ : ج تبقى بترتيب العربية", "فما نسبة أ : ج؟", null],
 ["معادلة بمتغيّر عربي تبقى بترتيب العربية", "إذا كان 7 − 2س = 1، فما قيمة س؟", null],
];
